...
from lib.scraper import trophy_fetcher


def scrape(id_to_psn, fetcher):
    id_to_trophies = {}
    ...
    for id, psn in id_to_psn.items():
        ...
        try:
            id_to_trophies[id] = asdict(fetcher(psn))
        except Exception:
            logging.critical(
...
def run(
    ...
    fetcher = trophy_fetcher(ps_token)
    ...
    while True:
        try:
            ...
            id_to_trophies = scrape({instant_id: instant_psn}, fetcher)
        ...
        if (time() - start_time) >= (refresh_interval_hours * 60 * 60):
            ...
            id_to_trophies = scrape(id_to_psn_batch_get(), fetcher)
